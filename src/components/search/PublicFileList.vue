    ...
    <div class="rounded-lg p-2 bg-gray-50 overflow-hidden">
        <table class="min-w-full table-fixed border-collapse rounded-lg">
            <thead class="bg-blue-100 border-none">
                <tr>
                    <th class="text-left text-sm uppercase tracking-wider text-gray-500 py-1.5 px-2 w-[45%]">{{ files.header[0] }}</th>
                    ...
                    <th class="text-left text-sm uppercase tracking-wider text-gray-500 py-1.5 px-2 w-[25%]">{{ files.header[3] }}</th>
                    <th class="text-left text-sm uppercase tracking-wider text-gray-500 py-1.5 px-2 w-[25%]">{{ files.header[4] }}</th>
                </tr>
            </thead>

            <tbody class="divide-y divide-gray-200">
                <tr v-if="canGoBack" @click="$emit('back')" class="cursor-pointer even:bg-blue-50 hover:bg-violet-200 transition-colors rounded! border-none">
                ...
                <tr v-else v-for="file in files.data" :key="file.name" :class="['even:bg-blue-50', 'hover:bg-violet-200', 'transition-colors', 'rounded!', 'border-none']">
                    <td class="py-1.5 px-2 overflow-hidden whitespace-nowrap text-ellipsis" :title="file.name">
                        <span v-if="file.type == 'folder'" class="flex items-center gap-x-2 text-gray-800">
                            <Link @click="$emit('goToFolder', file.file_id, file.name)" class="flex items-center gap-x-2 hover:bg-blue-100! px-0!">
                                <Folder class="text-amber-400 fill-amber-200" :stroke-width="1.5" />
                                {{ file.name }}
                            </Link>
                        </span>
                    ...
                    <td v-else class="py-1.5 px-2 text-base flex">
                        <!-- Folders don't have download option in public view -->
                    </td>
                </tr>
            </tbody>
        </table>
